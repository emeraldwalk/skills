...
# Usage (from project root): bash <SKILL_PATH>/scripts/pb-init.sh <module-name> <port> <admin-email> <admin-password>
set -e
...
if [ ! -f "$ENV_FILE" ]; then
  if [ -z "$PB_PORT" ] || [ -z "$PB_ADMIN_EMAIL" ] || [ -z "$PB_ADMIN_PASSWORD" ]; then
    ...
    echo "Usage (from project root): bash <SKILL_PATH>/scripts/pb-init.sh <module-name> <port> <admin-email> <admin-password>"
    exit 1
  fi
  cat > "$ENV_FILE" << EOF
PB_PORT=$PB_PORT
